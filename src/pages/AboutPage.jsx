import { Link } from "react-router-dom";
import { useI18n } from "../context/I18nProvider";
import {
  FaSeedling,
  FaUsers,
  FaCloudSun,
  FaStore,
  FaCheck,
} from "react-icons/fa";
const teamMembers = [
  {
    id: 1,
    name: "Sahil Patil",
  },
  {
    id: 2,
    name: "Sahil Belchada",
  },
  {
    id: 3,
    name: "Samar Prakash",
  },
];

const AboutPage = () => {
  const { t } = useI18n();
  return (
    <div>
      <section className="relative h-screen flex items-center justify-center overflow-hidden bg-gradient-to-b from-green-50 to-white">
        <div
          aria-hidden="true"
          className="pointer-events-none absolute inset-0 z-0"
          style={{
            backgroundImage:
              "linear-gradient(rgba(0,0,0,0.02) 1px, transparent 1px), linear-gradient(90deg, rgba(0,0,0,0.02) 1px, transparent 1px)",
            backgroundSize: "32px 32px",
          }}
        />
        <div className="relative z-10 w-full">
          <div className="max-w-4xl mx-auto text-center px-4">
            <div className="inline-block bg-green-100 text-green-800 text-xs font-semibold rounded-full px-3 py-1 mb-6 shadow-sm border border-green-200">
              <span className="uppercase tracking-wider">{t("ourStory")}</span>
            </div>
            <h1 className="text-4xl md:text-6xl font-extrabold mb-6 text-gray-900">
              {t("aboutHarit")}
            </h1>
            <p className="text-lg md:text-xl text-gray-600 mb-10">
              {t("aboutSubtitle")}
            </p>
          </div>
        </div>
      </section>

      <div className="container  mx-auto px-4 py-12">
        <section className="py-24 bg-gradient-to-b from-gray-50 to-white">
          <div className="container mx-auto px-4">
            <div className="bg-white/70 backdrop-blur-md shadow-xl p-10 rounded-3xl max-w-6xl mx-auto">
              <div className="flex flex-col md:flex-row items-center gap-12">
                <div className="md:w-1/2 text-center md:text-left">
                  <h2 className="text-4xl font-extrabold text-green-700 mb-6 leading-tight">{t("ourMission")}</h2>
                  <p className="text-gray-800 text-lg mb-4">
                    {t("missionP1")}
                  </p>
                  <p className="text-gray-800 text-lg">
                    {t("missionP2")}
                  </p>
                </div>

                <div className="md:w-1/2 flex justify-center">
                  <div className="w-60 h-60 bg-green-100 rounded-full flex items-center justify-center shadow-lg hover:scale-105 transition-transform duration-300">
                    <FaSeedling className="text-green-500 text-8xl" />
                  </div>
                </div>
              </div>
            </div>
          </div>
        </section>

        <section className="mb-24">
          <h2 className="text-3xl font-bold text-center mb-12 text-gray-900">{t("howItWorks")}</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
            <div className="glass p-8 rounded-2xl text-center shadow-md animate-fade-in">
              <div className="w-16 h-16 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-4">
                <FaSeedling className="text-green-500 text-3xl" />
              </div>
              <h3 className="text-xl font-semibold mb-2">{t("smartGuidance")}</h3>
              <p className="text-gray-600">
                {t("smartGuidanceDesc")}
              </p>
            </div>
            <div className="glass p-8 rounded-2xl text-center shadow-md animate-fade-in animate-delay-100">
              <div className="w-16 h-16 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-4">
                <FaCloudSun className="text-green-500 text-3xl" />
              </div>
              <h3 className="text-xl font-semibold mb-2">{t("weatherAlerts")}</h3>
              <p className="text-gray-600">
                {t("weatherAlertsDesc")}
              </p>
            </div>
            <div className="glass p-8 rounded-2xl text-center shadow-md animate-fade-in animate-delay-200">
              <div className="w-16 h-16 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-4">
                <FaStore className="text-green-500 text-3xl" />
              </div>
              <h3 className="text-xl font-semibold mb-2">{t("marketplace")}</h3>
              <p className="text-gray-600">
                {t("marketplaceDesc")}
              </p>
            </div>
          </div>
        </section>

        <section className="mb-24">
          <h2 className="text-3xl font-bold text-center mb-12 text-gray-900">{t("benefits")}</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            <div className="glass p-8 rounded-2xl shadow-md">
              <h3 className="text-xl font-semibold mb-4 text-green-600">{t("forConsumers")}</h3>
              <ul className="space-y-3 text-gray-700">
                <li className="flex items-start">
                  <FaCheck className="text-green-500 mt-1 mr-2" />
                  <span>{t("consB1")}</span>
                </li>
                <li className="flex items-start">
                  <FaCheck className="text-green-500 mt-1 mr-2" />
                  <span>{t("consB2")}</span>
                </li>
                <li className="flex items-start">
                  <FaCheck className="text-green-500 mt-1 mr-2" />
                  <span>{t("consB3")}</span>
                </li>
                <li className="flex items-start">
                  <FaCheck className="text-green-500 mt-1 mr-2" />
                  <span>{t("consB4")}</span>
                </li>
                <li className="flex items-start">
                  <FaCheck className="text-green-500 mt-1 mr-2" />
                  <span>{t("consB5")}</span>
                </li>
              </ul>
            </div>
            <div className="glass p-8 rounded-2xl shadow-md">
              <h3 className="text-xl font-semibold mb-4 text-green-600">{t("forFarmers")}</h3>
              <ul className="space-y-3 text-gray-700">
                <li className="flex items-start">
                  <FaCheck className="text-green-500 mt-1 mr-2" />
                  <span>{t("farmB1")}</span>
                </li>
                <li className="flex items-start">
                  <FaCheck className="text-green-500 mt-1 mr-2" />
                  <span>{t("farmB2")}</span>
                </li>
                <li className="flex items-start">
                  <FaCheck className="text-green-500 mt-1 mr-2" />
                  <span>{t("farmB3")}</span>
                </li>
                <li className="flex items-start">
                  <FaCheck className="text-green-500 mt-1 mr-2" />
                  <span>{t("farmB4")}</span>
                </li>
                <li className="flex items-start">
                  <FaCheck className="text-green-500 mt-1 mr-2" />
                  <span>{t("farmB5")}</span>
                </li>
              </ul>
            </div>
          </div>
        </section>

        <section className="mb-32">
          <h2 className="text-4xl font-bold text-center mb-4 text-gray-900">{t("meetTeam")}</h2>
          <p className="text-center text-gray-600 text-xl mb-16">
            {t("passionateTeam")}
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 max-w-2xl mx-auto">
            {teamMembers.map((member) => (
              <div
                key={member.id}
                className="glass p-6 rounded-2xl flex flex-col items-center shadow-xl hover:shadow-2xl transition-all duration-300 group border border-white/20"
              >
                <div className="w-20 h-20 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-6">
                  <FaUsers className="text-green-500 text-3xl" />
                </div>
                <h3 className="text-lg font-semibold text-gray-900 text-center">
                  {member.name}
                </h3>
              </div>
            ))}
          </div>
        </section>

        <section className="text-center mb-8">
          <h2 className="text-3xl font-bold mb-6 text-gray-900">{t("joinCommunity")}</h2>
          <p className="text-xl text-gray-500 max-w-3xl mx-auto mb-8">
            {t("joinCommunityDesc")}
          </p>
          <div className="flex flex-col sm:flex-row justify-center gap-4">
            <Link
              to="/register"
              className="btn btn-primary px-8 py-3 text-lg font-bold shadow-md"
            >
              {t("signUpNow")}
            </Link>
            <Link
              to="/products"
              className="btn btn-outline px-8 py-3 text-lg font-bold"
            >
              {t("browseProductsCta")}
            </Link>
          </div>
        </section>
      </div>
    </div>
  );
};

export default AboutPage;
